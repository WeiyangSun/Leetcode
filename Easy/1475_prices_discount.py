...
class Solution:
    def finalPrices(self, prices: list(int)) -> list(int):
        ...
        for i in range(n):
            ...
            search_flag = True
            for j in range(i, n):
                if j > i and prices[j] <= prices[i] and search_flag:
...
class Solution:
    def finalPrices(self, prices: list(int)) -> list(int):
        ...
        for ix, v in enumerate(prices):
            while index_stack and prices[index_stack[-1]] >= v:
                prices[index_stack.pop()] -= v
            index_stack.append(ix)
