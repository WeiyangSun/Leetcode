...
class RecentCounter:
    ...
    def __init__(self):
        self.requests = deque()  # empty deque to store request times

    def ping(self, t: int) -> int:
        self.requests.append(t)  # adds new request time to the right end of queue
        while self.requests[0] < t - 3000:
            self.requests.popleft()  # removes oldest request if outside of window
        return len(self.requests)  # number of requests in window
