...
result be palced in the first part of the array nums. More formally, if there are k elements after removing
the duplciates, then the first k elements of nums should hold the final result. It does not matter what
...
class Solution:
    def removeDuplicates(self, nums: list[int]) -> int:
        ...
        for fast_pointer in range(2, len(nums)):
            if nums[fast_pointer] != nums[slow_pointer - 2]:
