    ...
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for i in range(len(s)):
            if s[i] in seen and seen[s[i]] >= start:
                ...
                start = seen[s[i]] + 1  # Move the start to the next position after the repeated character
            seen[s[i]] = i  # Update the character's most recent index
            global_max = max(global_max, i - start + 1)  # Calculate the length of the current window
