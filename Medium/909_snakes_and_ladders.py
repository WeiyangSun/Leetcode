...
a Boustrophedon style starting from the bottom left of the board (i.e. board[n - 1][0]) and
...
class Solution:
    def snakesAndLadders(self, board: List[List[int]]) -> int:
        ...
        def get_coords(step_no):
            r = (step_no - 1) // n  # zero-based row index from bottom
            c = (step_no - 1) % n  # zero-based col index from left
            row = n - 1 - r  # convert to top-down indexing (normal operation)

            if r % 2 == 1:  # if the row is odd, (0-indexing)
                col = n - 1 - c  # flip col
            else:
                col = c
            return row, col
        ...
        while queue:
            for _ in range(len(queue)):
                ...
                for nxt in range(curr + 1, min(curr + 6, n * n) + 1):  # try dice rolls 1 to 6
                    row, col = get_coords(nxt)
                    dest = (
                        board[row][col] if board[row][col] != -1 else nxt
                    )  # snake jump / ladder slides
