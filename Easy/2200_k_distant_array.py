...
and k. A k-distant index is an index i of nums for which there exists
...
where |0 - j| <= k and nums[j] == key. Thus, 0 is not a k-distant index.
...
that |i - j| <= k and nums[j] == key, so every index is a k-distant index. 
